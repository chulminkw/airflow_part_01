...
TARGET_TABLE = "orders_daily_summary_point_wd_skip" # weekday용 summary table 신규 생성
LOCAL_TZ = "Asia/Seoul" # 또는 pendulum.timezone("ASIA/SEOUL")

# 주말외에 수행되지 말아야 할 공휴일(보통은 DB로 관리)
# 아래는 2026년 설날 연휴일자 + 1. 실행 logical_date는 추출 데이터 구간 기준 일자 + 1
DO_NOT_RUN_HOLIDAYS = [
    datetime(2026, 2, 17, hour=0, minute=0, second=0, tz=LOCAL_TZ ),
    datetime(2026, 2, 18, hour=0, minute=0, second=0, tz=LOCAL_TZ ),
    datetime(2026, 2, 19, hour=0, minute=0, second=0, tz=LOCAL_TZ )              
]

with DAG(
    dag_id="cron_point_wd_skip_01",
    start_date=datetime(2026, 2, 6, 
                        hour=0, minute=0, second=0, tz=LOCAL_TZ), # 영상을 보시는 날 이전으로 주말이 포함될 수 있게 설정해 주세요
    schedule=CronTriggerTimetable("0 0 * * *", timezone=LOCAL_TZ), # DAG start_date의 tz와 동일한 timezone 설정.
    ...
    def check_if_run(**context):
        ...
        # 실행 logical_date는 추출 데이터 구간 기준 일자 + 1
        # 만약 실행 logical_date가 일요일과 월요일이면 아래는 AirflowSkipException을 발생 시키고 더 이상 task를 수행하지 않음
        ...
        if logical_date_tz.day_of_week in [pendulum.SUNDAY, pendulum.MONDAY]:
    ...
    def load_orders_daily_summary(**context):
        ...
        logical_date_tz = logical_date.in_timezone(timezone_value)
